...
@section('contents')
    <form action="{{ route('admin.mcp-kpk.sub-indikator.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            ...
            <select name="mcp_indikator_id" class="form-control @error('mcp_indikator_id') is-invalid @enderror shadow-none">
                ...
                @foreach ($mcp_indikator as $i)
                    <option value="{{ $i->id }}">{{ $i->area_intervensi->keterangan ?? '' }} - {{ $i->keterangan }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="obrik_id" class="form-control @error('obrik_id') is-invalid @enderror shadow-none">
                <option value="">Pilih</option>
                @foreach ($obrik as $i)
                    <option value="{{ $i->id }}">{{ $i->bidang_obrik ?? '' }}
                    </option>
                @endforeach
            </select>
            @error('obrik_id')
